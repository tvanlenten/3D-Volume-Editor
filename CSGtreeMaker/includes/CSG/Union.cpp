...
CSGnodeStatus Union::displayGUI() {
	char treeTag[32], removeTag[32], addTag[32], popUpTag[32];
	snprintf(treeTag, 32, "Union###tree%d", id);
...
void Union::getCSGtree(std::vector<CSGelement> tree) {
}

std::string Union::getCSGtreeHelper(int index) {
	if (index == args.size() - 1) {
		return args[index]->getCSGtree();
	} else {
		std::string str = "U(";
		std::string arg1 = (args.size() > 0) ? args[index]->getCSGtree() : "0.0";
		std::string arg2 = (args.size() > 1) ? getCSGtreeHelper(index + 1) : "0.0";
		str = str + arg1 + ",\n" + arg2 + ")";
		return str;
	}
}

std::string Union::getCSGtree() {
	return getCSGtreeHelper(0);
}
